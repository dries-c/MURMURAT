...
func testRSA() {
	...
	packetHandler.RegisterListener(message.IDData, func(msg message.Message) error {
		...
		publicKey := handler.RSAPublicKeyFromBytes(publicKeyBytes, 65537)
		signatureVerifier := handler.NewSignatureVerifier(publicKey)

		err = signatureVerifier.Verify(dataMessage.Data, dataMessage.Signature)
		if err != nil {
